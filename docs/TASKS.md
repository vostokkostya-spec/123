...
- Local model inventory verified at 12:55 MSK: four generation models plus two embedding models, all installed; embedding models remain excluded from generation. Ollama Cloud is authenticated as `vostokkostya` and `gpt-oss:20b-cloud` passed a live test; cloud routing remains explicit opt-in.
